...
description: EEARS 預約/候補/取消流程與 2hr 截止（含時間窗、blacklist、問卷 Gate）
license: MIT
---
...
- 你要改任何學生端預約、候補、取消（含 UI 提示或 API 錯誤碼）
...
- 候補加入：`reservation-backend/routes/eventRouter.js`
...
- [ ] 若涉及 waitlist：
  - 是否仍需在「額滿」情境才允許加入？
  - 是否與正式預約共用 blacklist/時間窗/問卷 gate/duplicate 邏輯？
- [ ] 若涉及 error codes：
...
- 在候補流程中不套用 blacklist（或在正式預約中套用了但候補沒套用）

## Suggested Workflow Prompt

> 請先列出我這個需求會影響哪些規則（2hr/開放窗/blacklist/問卷 Gate/waitlist 允許條件/duplicate），並告訴我你會優先讀取哪些檔案；確認後再提出具體改動建議與需補的測試項目。
