...
export const SURVEY_WORKFLOW_STEPS = [
  ...
  },
  {
    id: 'publish',
    title: '編輯題目並發布',
    summary: '建立草稿（自動帶入上一版）→ 視覺化編輯題目與即時預覽 → 發布；學生端只會看到「已發布」版本。',
    path: '/admin/survey-center',
    action: '在列表按「編輯與發布」',
  },
  {
...
export const SURVEY_PAGE_GUIDES = {
  center: {
    ...
    bullets: [
      ...
      '編輯題目：在該列按「編輯題目與發布」→ 建立草稿 → 開啟視覺化編輯器（右側可即時預覽學生端）→ 儲存 → 發布。',
      '刪除：操作選單可封存／刪除問卷；草稿版本可在版本列表刪除（已發布版不可刪）。有作答的問卷會先封存以保留資料。',
      '檢視／匯出作答：側欄「作答紀錄」「統計分析」，或列表「操作」進入。',
      '複製公開連結：操作選單「複製公開填答連結」，可貼到首頁／海報／訊息（路徑／survey/{問卷代碼}）。',
      '讓學生何時要填：請至側欄「啟用規則」設定學期與活動條件（含 PUBLIC 獨立問卷）。',
    ],
  },
  rules: {
    title: '啟用規則怎麼用？',
    intro: '問卷建立並發布後，在此指定「哪個學期、哪類活動、何時要填哪份問卷」。活動類型可選 PUBLIC 讓問卷出現在首頁選擇頁，且不擋預約。',
    bullets: [
      '新增規則：按「新增規則」，選學期、活動類型、問卷與版本。',
      'ET／EC 必填規則會驅動預約 Gate；問卷代碼以規則綁定為準（不再死綁單一學期 key）。',
      '獨立／首頁問卷：活動類型選「PUBLIC」，啟用後會出現在／survey/choice，不擋預約。',
      '編輯／刪除：在列表右側「編輯」「刪除」。',
