...
/** 數據分析：Q21 宣傳來源、系所、年級（可指定學期） */
export async function fetchEnglishTestAnalyticsStats(token, { semester } = {}) {
  const qs = new URLSearchParams();
  if (semester != null && String(semester).trim() !== '') {
    qs.set('semester', String(semester).trim());
  }
  const suffix = qs.toString() ? `?${qs.toString()}` : '';
  const res = await fetchClient(`${BASE}/stats/analytics${suffix}`, { headers: authHeaders(token) });
  const data = await parseJson(res);
  if (!res.ok) throw new Error(data.error || '載入統計失敗');
  return {
    ...
    availableSemesters: Array.isArray(data.availableSemesters) ? data.availableSemesters : [],
    semesterCounts:
      data.semesterCounts && typeof data.semesterCounts === 'object' && !Array.isArray(data.semesterCounts)
        ? data.semesterCounts
        : {},
  };
}
