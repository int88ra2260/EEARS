...
const PATHS = {
  ...
  englishTestRegistrationEdit: '/api/settings/english-test-registration-edit-enabled',
};
...
export async function fetchEnglishTestRegistrationEditEnabledPublic() {
  return fetchPublicBooleanSetting(PATHS.englishTestRegistrationEdit, true);
}
...
export async function fetchSystemSettingsBundle(token, userRole) {
  ...
  const tasks = await Promise.allSettled([
    ...
    fetchClient(PATHS.englishTestRegistrationEdit, { headers }).then((r) => r.json()),
  ]);
  ...
  return {
    ...
    englishTestRegistrationEditEnabled: tasks[2].status === 'fulfilled' ? tasks[2].value.enabled !== false : true,
