...
> 狀態：Phase 1.2 已完成（含匯出、範圍條件精簡）  
> 對齊模組：LVA（`learning-analytics`）  
> 語氣底線：觀察關聯，非因果證明（`causalClaimAllowed = false`）  
> 現況總報告：`docs/learning-analytics-status-report.md`
...
- 有進步人數（任一前後測／全技能等定義）
- **進步／持平／退步人數**（依學生層級前後測平均：>0／=0／<0）
- **平均原始分進步**（`deltaRawScore`，與現有「資源效益」頁口徑一致：先算學生平均，再算群體平均）— **同測主指標**
- **中位數與 Q1–Q3**（同一學生層級平均分布），避免只看平均被 1～2 人拉高
- **易受極端值影響**警示（平均相對中位偏離，或剔除最極端 1 人後平均明顯改變）
- **GSE 實際／修正**（跨測驗量尺；同測時為輔）。原始明顯變但 GSE≈0 時標「換算解析度不足」
- 進步定義文案：任一技能進步／全技能進步（四科齊備）／學生平均原始進步 > 0
- 進步率 = 有進步人數 ÷ 可計算成長人數

進步定義（產品口徑）：

1. **任一技能進步**：聽／說／讀／寫中至少一項有前後測，且該項後測優於前測  
2. **全技能進步**：聽／說／讀／寫**四項都有**前後測，且每一項後測都優於前測（缺科不算）

成長量尺原則（contract `offerings.v4`）：前後測配對僅在同一英檢內；同測看原始分，GSE 用於跨測驗比較。培力錨點依 CEFR 分數門檻對齊 GSE 帶（見 `docs/learning-analytics-gse-anchor-audit.md`）。

### 非目標（Phase 1 不做）

- 不作「此課程／教師**造成**進步」的因果宣稱
- 不作教師績效排名對外公布（樣本過小需遮蔽）
- 不恢復舊「教師影響（proxy）」實驗頁；課／師／活動成效以本模組的描述性分析為準
...
- 可計算成長人數 **&lt; 10**（`MIN_GROWTH_SAMPLE`）：遮蔽平均進步、分布、方向人數與進步率，顯示「樣本不足」
...
| 舊教師影響（proxy） | 已自後台移除；避免與課／師／活動描述性分析混淆 |
