...
const OFFERING_SUMMARY_COLUMNS = [
  ...
  { key: 'directionImproved', header: '進步人數(學生平均>0)', width: 14 },
  { key: 'directionFlat', header: '持平人數(學生平均=0)', width: 14 },
  { key: 'directionDeclined', header: '退步人數(學生平均<0)', width: 14 },
  { key: 'avgRawDelta', header: '平均原始分進步', width: 14 },
  { key: 'medianRawDelta', header: '中位原始分進步', width: 14 },
  { key: 'rawP25', header: '原始分Q1', width: 10 },
  { key: 'rawP75', header: '原始分Q3', width: 10 },
  { key: 'avgActualGseGrowth', header: 'GSE 實際成長', width: 12 },
  { key: 'medianActualGseGrowth', header: 'GSE 實際中位', width: 12 },
  { key: 'avgAdjustedGseGrowth', header: 'GSE 修正成長', width: 12 },
  { key: 'medianAdjustedGseGrowth', header: 'GSE 修正中位', width: 12 },
  { key: 'primaryGrowthMetric', header: '主指標', width: 12 },
  { key: 'gseResolutionWarningStudentCount', header: 'GSE解析度警示人數', width: 16 },
  { key: 'outlierSkewFlagged', header: '易受極端值影響', width: 14 },
  { key: 'outlierSkewReason', header: '極端值說明', width: 48 },
...
const STUDENT_DETAIL_COLUMNS = [
  ...
  { key: 'avgActualGseGrowth', header: 'GSE 實際成長', width: 12 },
  { key: 'avgAdjustedGseGrowth', header: 'GSE 修正成長', width: 12 },
  { key: 'improvedAny', header: '任一技能進步', width: 14 },
  { key: 'improvedAllSkills', header: '全技能進步(四科)', width: 14 },
  { key: 'instruments', header: '英檢工具', width: 16 },
  { key: 'gseResolutionWarning', header: 'GSE解析度警示', width: 14 },
  { key: 'gseResolutionReason', header: 'GSE警示說明', width: 48 },
];
...
function flattenOfferingSummaryRow(row, dimension) {
  return {
    ...
    directionImproved: row.direction?.improved ?? '',
    directionFlat: row.direction?.flat ?? '',
    directionDeclined: row.direction?.declined ?? '',
    avgRawDelta: row.avgRawDelta ?? '',
    medianRawDelta: row.medianRawDelta ?? row.rawDistribution?.median ?? '',
    rawP25: row.rawDistribution?.p25 ?? '',
    rawP75: row.rawDistribution?.p75 ?? '',
    avgActualGseGrowth: row.avgActualGseGrowth ?? '',
    medianActualGseGrowth: row.medianActualGseGrowth ?? row.gseActualDistribution?.median ?? '',
    avgAdjustedGseGrowth: row.avgAdjustedGseGrowth ?? '',
    medianAdjustedGseGrowth: row.medianAdjustedGseGrowth ?? row.gseAdjustedDistribution?.median ?? '',
    primaryGrowthMetric: row.primaryGrowthMetric || 'rawDelta',
    gseResolutionWarningStudentCount: row.gseResolutionWarningStudentCount ?? '',
    outlierSkewFlagged: row.outlierSkew?.flagged ?? false,
    outlierSkewReason: row.outlierSkew?.reason || '',
...
function flattenStudentDetailRows(offeringKey, offeringLabel, students) {
  return (students || []).map((student) => ({
    ...
    instruments: (student.instruments || []).map((item) => `${item.instrument}:${item.episodeCount}`).join('；'),
    gseResolutionWarning: student.gseResolutionWarning?.flagged ? '是' : '',
    gseResolutionReason: student.gseResolutionWarning?.reason || '',
  }));
}
